function [v_ans,next_minmax] = CurrentSafestVelocity(secindNew, mapSecToORCA, ORCA, n, coordintsec, nintsec, dirintsec, valintsec, v_safest, prev_minmax, v_max)
    ...
    %% Check if lowest value can be maintained but on new line
    
    %if abs(dot(coordintsec(secindNew,:)-ORCA(mapSecToORCA(secindNew),:),n(mapSecToORCA(secindNew)))) < tol
%     if valintsec(secindNew) ~= 0
%         v_ns = coordintsec(secindNew,:) - prev_minmax*dirintsec(secindNew,:); %Something is wrong.
%         
%         % Check if v_ns is allowed
%         newAllowed = true;
%         for i = 1:secindNew-1
%             if (dot((v_ns - coordintsec(i,:)),nintsec(i,:)) < 0)
%                 newAllowed = false;
%                 break;
%             end
%         end
...
%     end
    ...
    while (secind < secindNew)
        ...
        if (EXITFLAG > 0)
            ...
            npoints(lineind,:) = nintsec(secind,:); %Räcker det med map?
    ...
    for j = 1:size(coordpoints,1)
        ...
        if (dir > 0)
            ...
            if (~minSet)
                ...
                if dot((min_v-coordpoints(j,:)),dirintsec(secindNew,:)) > 0 %dirpoints(j,:)?
                    min_v = coordpoints(j,:);
                    minind = j;
                end
            end
    ...
    if norm(v_ans) > v_max
        v_ans = CheckSpeed(v_ans, nintsec(secindNew,:), v_max); % TODO, borde det vara nintsec istället för dirintsec?
    end
